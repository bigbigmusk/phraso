...
4. 运行测试：`⌘U`（`PhrasoTests` target 覆盖词块判定、复习调度、口语转写判定和内容包完整性校验）。
...
└── Resources/
    └── Content/    fr / es / de / ko 语言包 JSON（每包 12 个引擎，前 8 个含完整课程）
PhrasoTests/        单元测试：判定器 / 调度器 / 语音评估 / 内容包不变量
```

## 内容说明

- 每个语言包定义 12 个句子引擎；引擎 1–8 已含完整可玩课程（表达意愿、自我介绍、有、喜欢、餐厅点餐、提问、否定、时间表达——原创内容，四语共享教学框架但**不共享机械翻译**：德语突出句框与动词提前问句，韩语按助词/语尾/量词/省略逻辑教学），引擎 9–12 为 `coming_soon` 占位，供内容团队按 PRD 16 的生产流程补齐至 48 节。
- 内容不变量由单元测试守护：词块拼接必须与参考答案 token 一致、干扰词不与正确词块重复、每课先理解后构句且以迁移题收尾、韩语参考句必须带罗马字。
